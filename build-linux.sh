...
STD_FLAGS="-std=c99 -std=c++14"
COMPILATION_FLAGS="$STD_FLAGS -Os -flto"
FINAL_COMPILE_FLAGS="-s"
WARNING_FLAGS="-Wall -Wextra -Wpedantic -Wno-unknown-pragmas"
...
if [ -n "$BUILD_DEBUG" ]; then
    ...
    COMPILATION_FLAGS="$STD_FLAGS -O0 -g"
    FINAL_COMPILE_FLAGS=""
...
if [ ! -d "$TEMP_DIR" ]; then
    ...
    if [ -n "$REALLY_QUIET" ]; then
        $GCC -c $RAYLIB_DEFINES $RAYLIB_INCLUDE_FLAGS $COMPILATION_FLAGS $RAYLIB_C_FILES > /dev/null 2>&1
    else
        $GCC -c $RAYLIB_DEFINES $RAYLIB_INCLUDE_FLAGS $COMPILATION_FLAGS $RAYLIB_C_FILES
    fi
